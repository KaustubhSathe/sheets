...
func handleRequest(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	// Now create a new comment in DB
	// PK: spreadsheet id
	// SK: comment id
	comment, err := dynamo.CreateComment(body.SpreadSheetID, int64(userInfo.User["id"].(float64)), userInfo.User["login"].(string), body.SheetNo, body.CellID, body.Content)
	if err != nil {
		return events.APIGatewayProxyResponse{
			StatusCode: 500,
		}, err
	...
	var stringified = comment.Stringify()
